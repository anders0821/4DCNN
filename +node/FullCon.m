classdef FullCon < handle
    ...
    methods
        function [this] = FullCon(pI, OUTPUTCHANNEL)
            ...
            this.O = zeros([1 1 1 1 OUTPUTCHANNEL N]);
        ...
        function [] = ff(this)
            ...
            %tic;
            this.O = this.P * permute(this.pI.O, [5 6 1 2 3 4]);
            this.O = permute(this.O, [3 4 5 6 1 2]);
            %toc;
        end
        ...
        function [] = bp(this)
            ...
            % 快速计算激活的梯度
            J = this.P;
            this.pI.gradO = J' * permute(this.gradO, [5 6 1 2 3 4]);% 单个输入样本的梯度
            this.pI.gradO = permute(this.pI.gradO, [3 4 5 6 1 2]);
            
            % 计算参数的梯度
            [H, W, D, T, INPUTCHANNEL, N] = size(this.pI.O);
            [H, W, D, T, OUTPUTCHANNEL, N] = size(this.O);
            this.gradP = zeros(OUTPUTCHANNEL, INPUTCHANNEL);
            for i=1:N
                this.gradP = this.gradP + permute(this.gradO(:,:,:,:,:,i), [5 6 1 2 3 4]) * permute(this.pI.O(:,:,:,:,:,i), [5 6 1 2 3 4])';
            end
