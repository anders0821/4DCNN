classdef SoftMaxNorm < handle
    ...
    methods
        ...
        function [] = ff(this)
            ...
            [H, W, D, T, C, N] = size(this.pI.O);
            num = exp(this.pI.O);
            den = sum(exp(this.pI.O), 5);
            den = repmat(den, [1 1 1 1 C 1]);
            this.O = num ./ den;
            %toc;
        end
        ...
        function [] = bp(this)
            ...
            N = size(this.O, 6);
            for i=1:N
                vec = permute(this.pI.O(:,:,:,:,:,i), [5 6 1 2 3 4]);% 单个输入样本
                tmp = exp(vec) / sum(exp(vec));
                J = diag(tmp) - tmp*tmp.';% 单个输入样本对单个输出样本的J
                this.pI.gradO(:,:,:,:,:,i) = J' * permute(this.gradO(:,:,:,:,:,i), [5 6 1 2 3 4]);% 单个输入样本的梯度
            end
