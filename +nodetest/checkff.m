function [loss] = checkff(MINIBATCHSIZE, c0P, c1P, cvP, b2P, d5mask, d7mask, f9P, b10P, xO, c0O, c1O, cvO, t3O, s4O, d5O, d6O, flatO, d7O, d8O, b10O, smnO, yO, smlO, offsetId, offset);
    ...
    for i=1:MINIBATCHSIZE% N
        for j=1:2% 输出通道
            for k=1:3% 输入通道
                tmp = convn(checkLast(:,:,:,:,k,i), c0P(:,:,:,:,k,j), 'same');
    ...
    % conv
    checkLast = checkVal;
    checkVal = zeros([16 12 3 4 3 MINIBATCHSIZE]);% 输出尺寸
    for i=1:MINIBATCHSIZE% N
        for j=1:3% 输出通道
            for k=1:2% 输入通道
                tmp = convn(checkLast(:,:,:,:,k,i), c1P(:,:,:,:,k,j), 'same');
                checkVal(:,:,:,:,j,i) = checkVal(:,:,:,:,j,i) + tmp;
            end
        end
    end
    ...
    % convvalid
    checkLast = checkVal;
    checkVal = zeros([14 10 1 2 4 MINIBATCHSIZE]);% 输出尺寸
    for i=1:MINIBATCHSIZE% N
        for j=1:4% 输出通道
            for k=1:3% 输入通道
                tmp = convn(checkLast(:,:,:,:,k,i), cvP(:,:,:,:,k,j), 'valid');
                checkVal(:,:,:,:,j,i) = checkVal(:,:,:,:,j,i) + tmp;
            end
        end
    end
    ...
    assert(all( size(checkVal)==[7 5 1 1 4 4] ));
    if(~isempty(s4O))
    ...
    % dropoutTrain
    assert(all(all(all(all(all(all( repmat(d5mask(1,1,1,1,:,:), [7 5 1 1 1 1])==d5mask )))))))
    ...
    % flatten
    checkVal = reshape(checkVal, [1 1 1 1 7*5*1*1*4 MINIBATCHSIZE]);
    if(~isempty(flatO))
    ...
    % dropoutTest
    checkVal = checkVal*(round(140*0.44)/140);
    if(~isempty(d8O))
