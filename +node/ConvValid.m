classdef ConvValid < handle
    ...
    methods
        function [this] = ConvValid(pI, KERNELWIDTHS, OUTPUTCHANNEL)
            ...
            for i=1:N
                for j=1:OUTPUTCHANNEL
                    for k=1:INPUTCHANNEL
                        tmp = convn(this.pI.O(:,:,:,:,k,i), this.P(:,:,:,:,k,j), 'valid');
        ...
        function [] = bp(this)
            ...
            for i=1:N
                for j=1:INPUTCHANNEL
                    for k=1:OUTPUTCHANNEL
                        tmp = convn(this.gradO(:,:,:,:,k,i), rotP(:,:,:,:,j,k), 'full');
            ...
            extGradO = zeros([H-size(this.P,1)+1+extBrd(1)*4 W-size(this.P,2)+1+extBrd(2)*4 D-size(this.P,3)+1+extBrd(3)*4 T-size(this.P,4)+1+extBrd(4)*4 OUTPUTCHANNEL N]);
