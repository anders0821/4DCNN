classdef ConvSameGpuStopBp < handle
    properties
        pI;
        P;
        O;
    ...
    methods
        function [this] = ConvSameGpuStopBp(pI, KERNELWIDTHS, OUTPUTCHANNEL)
        ...
        function [] = ff(this)
            ...
            thisO = zeros([H W D T OUTPUTCHANNEL N], 'gpuArray');
            for i=1:N
        ...
        function [] = bp(this)
            ...
            for i=1:N
                for j=1:INPUTCHANNEL
                    for k=1:OUTPUTCHANNEL
                        tmp = convn(thisgradO(:,:,:,:,k,i), rotP(:,:,:,:,j,k), 'same');
            ...
            extGradO = zeros([H+extBrd(1)*2 W+extBrd(2)*2 D+extBrd(3)*2 T+extBrd(4)*2 OUTPUTCHANNEL N], 'gpuArray');
            extGradO(1+extBrd(1):end-extBrd(1), 1+extBrd(2):end-extBrd(2), 1+extBrd(3):end-extBrd(3), 1+extBrd(4):end-extBrd(4), :, :) = gpuArray(this.gradO);
