classdef OpticalFlow5 < handle
    ...
    methods
        function [this] = OpticalFlow5(pI, shape)
            this.pI = pI;
            this.shape = shape;
            ...
            if(strcmp(this.shape,'same'))
                ...
                % 处理后通道为两个方向的光流1->5
                this.O = zeros([H W D T 5 N]);
                this.gradO = zeros(size(this.O));
            else
                this.O = zeros([H W D-1 T 5 N]);
                this.gradO = zeros(size(this.O));
            end
        end
        ...
        function [] = ff(this)
            ...
            assert(T==1);
            assert(C==1);
            of = zeros(H, W, D-1, T, 5, N);% of比输入少一帧 且5通道
            for i=1:N
                % 定义光流检测器
                ...
                for j=1:D
                    ...
                    if(j>=2)
                        ...
                        of(:,:,j-1,:,3,i) = -real(optical_flow_vector);
                        of(:,:,j-1,:,4,i) = -imag(optical_flow_vector);
                        of(:,:,j-1,:,5,i) = abs(optical_flow_vector);
                    end
                end
            end
